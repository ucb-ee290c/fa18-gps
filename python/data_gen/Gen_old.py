...
T = 5    # Data duration in time (seconds)
SV = [0, 5, 10, 15, 20, 25]
dopplerVaryRate = 10   # The artificially added doppler shift variation (Hz/s)

adcBitWidth = 3
dopplerRange = 5000      # Doppler shift range in Hz, in +/-
codePhaseRange = 1022    # Code phase shift range in unit of one PRN chip
#IF = 2.6e6                # IF in Hz
...
allInitialDoppler = np.random.randint(2*dopplerRange, size=len(SV))
allDopplerVaryDirection = np.random.randint(2, size=len(SV))*2 - 1

# Generate all doppler shift values for all samples
allDoppler = np.zeros((len(SV), totalDataLength)).astype(int)
for i in range (0, len(SV)):
    allDoppler[i] = (np.arange(allInitialDoppler[i], allInitialDoppler[i]+allDopplerVaryDirection[i]*T\
            *dopplerVaryRate, allDopplerVaryDirection[i]*dopplerStepPerSample)) - dopplerRange
...
for i in range(0, allSampledCarrier.shape[0]): 
...
# Add some noise
mixedDataBPSKNoise = mixedDataBPSK + (np.random.rand(np.shape(mixedDataBPSK)[0])-0.5)*1
...
adcOutputBPSKbin.tofile('ADCOutput.bin', sep='')

print("Writting random Doppler Shift into file 'DopplerShift.csv' ...")
(allInitialDoppler-dopplerRange).tofile('DopplerShift.csv', sep=',', format='%d')
...
allCodePhase.tofile('CodePhase.csv', sep=',', format='%d')
#(codePhaseRange*np.around(adcSamplingFreq/prnFreq)-allCodePhase).tofile('CodePhase2.csv', sep=',', format='%d')
...
with open('BasebandData.csv','w') as f:
    writer = csv.writer(f)
