...
class ExampleTopWithTracking(implicit p: Parameters) extends ExampleTop
    with HasPeripheryTracking {
  override lazy val module = new ExampleTopWithTrackingModule(this)
}

class ExampleTopWithTrackingModule(l: ExampleTopWithTracking)
  extends ExampleTopModule(l) with HasPeripheryTrackingModuleImp
